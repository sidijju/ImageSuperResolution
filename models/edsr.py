...
class EDSR:
    ...
    def train(self):
        ...
        plot_compare_batch(sample_hr, sample_lr, sample_hr_rec, self.progress_dir + f"comp:0")
        del sample_hr_rec
        ...
        for epoch in tqdm(range(self.args.n)):
            for i, batch in enumerate(tqdm(self.train_loader, 0)):
                ...
                if (iters % 5000 == 0) or ((epoch == self.args.n-1) and (i == len(self.train_loader)-1)):
                    ...
                    plot_compare_batch(sample_hr, sample_lr, batch_yhat, self.progress_dir + f"comp:{iters}")
